<template>
  <div>
    ...
    <form class="flex flex-col justify-center mx-auto px-9 md:w-1/2">
      ...
      <!-- WRITE A CAPTION FOR THE UPLOADED IMAGE -->
      <input
        v-if="showCaption"
        v-model="caption"
        type="text"
        placeholder="Write a caption"
        class="text-left pl-5 rounded-[32px] h-[41px] w-full border border-purple-500 fill-white my-10"
      />
      <button
        :disabled="disableButton"
        @click.prevent="upload"
        class="btn btn-outlined mx-7 mb-5"
      >
        Upload
      </button>
    </form>
...
const school_uuid = await client.getSchoolUuid(); // get school uuid
const uuid = client.school.uuid;
console.log(uuid);
console.log(school_uuid);
...
const user = useSupabaseUser(); //get auth user

// Handle the uplaod
const myUpload = (upload) => {
  const selectedFile = upload;
  console.log("Selected file:", selectedFile);
  const { name } = selectedFile; //destructure the name
...
// upload the file
const upload = async () => {
  disableButton.value = true; // disable the button
  ...
  try {
    ...
    if (!caption.value) {
      throw new Error("add a caption");
    }
    // upload the document
    const { data, error } = await supabase.storage
      .from("activities")
      .upload(school_uuid + "/" + name_of_file.value, file.value);
    if (data) {
      // add the caption to the activities table and url
      const { data: activityData, error: activityError } = await supabase
        ...
        .select();
      if (activityError) {
        checker(notify, 3000, activityError.message);
        // throw new Error(documentError.message);
        // delete the saved file

        const { data, error } = await supabase.storage
          .from("activities")
          .remove([school_uuid + "/" + name_of_file.value]);
        console.log(data);
        disableButton.value = false;
        throw new Error(activityError.message);
      }
      console.log("this is the activity data", data.path);
      successinfo(notify, 3000, "file uploaded");

      navigateTo("/teacher/activity");
    }
